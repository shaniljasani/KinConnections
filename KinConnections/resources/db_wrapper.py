...
airtable_connectees = Airtable(
    os.getenv('AIRTABLE_BASE_ID'), 'connectees', os.getenv('AIRTABLE_API_KEY'))
airtable_connectors = Airtable(
    os.getenv('AIRTABLE_BASE_ID'), 'connectors', os.getenv('AIRTABLE_API_KEY'))
airtable_auth = Airtable(os.getenv('AIRTABLE_BASE_ID'),
                         'auth', os.getenv('AIRTABLE_API_KEY'))
...
class db_wrapper:
    ...
    def login_auth(email, password):
        ...
        if matching_users_arr:
            ...
            auth_info_arr = airtable_auth.search(
    ...
    def signup_new_connectee(form_entries):
        ...
        try:
            ...
            logging.error(
                "There was an issue creating user record in user table")
            return None
        ...
        try:
            ...
            logging.error(
                "There was an issue creating auth record for user: " + str(user))
    ...
    def get_all_connectors():
        all = [connector['fields']
               for connector in airtable_connectors.get_all()]
        all_approved = []
